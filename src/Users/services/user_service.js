...
const registerUser = async({fname, lname, email, pass, role, gender})=> {
    ...
    const newuser = await User.create({fname, lname, email, pass:hasedPassword,  role, gender:genderr,passwordChangedAt:current_date,resetPasswordOtp:'',otpExpires:current_date});
...
const loginuser =async({email,pass})=>{
   const user = await User.findOne({email});
    ...
    const token= jwt.sign({id:user._id,email:user.email,role:user.role}, process.env.jwtsecrettoken,{expiresIn:'10h'});
